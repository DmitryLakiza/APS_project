...
requests_list = ['lam graph',
                 ...
                 'lam graph and model',
                 'analyzed line (requests completion of number 6 or 7)',
                 'histogram',
                 'end'
                 ]
...
while 1:
    ...
    if request == 'lam graph':
        lam_graph()
        plt.show()


    elif request == 'loglam graph':
        loglam_graph()
        plt.show()


    elif request == 'element':
    ...
    elif request == 'lam graph model':
        lam_graph_model()
        plt.show()
    elif request == 'loglam graph model':
        loglam_graph_model()
        plt.show()
    elif request == 'gauss lam graph':
    ...
    elif request == 'gauss lam graph model':
        question_Gauss_1 = input('Handmade or auto?: ')
        if question_Gauss_1 == 'Handmade' or question_Gauss_1 == 'handmade':
            while True:
                print('According to graph, find ends and "h": ')
    ...
    elif request == 'lam graph and model':
        fig = plt.subplot()
        x = np.linspace(min(needed_loglam), max(needed_loglam), len(needed_flux))
        fig = plt.plot(10 ** x, needed_flux[range(len(needed_flux))])
